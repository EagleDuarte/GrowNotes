...
const fillFields = (client) => {
  ...
  document.getElementById("detalhamento").value = client.detalhamento;
  document.getElementById("descricao").dataset.index = client.index;
};
  ...
  if (event.target.type == "button") {
    ...
    if (action == "edit") {
      ...
      const response = confirm(
        `Deseja realmente excluir o cliente ${client.descricao}`
      );
      if (response) {
        deleteClient(index);
        updateTable();
